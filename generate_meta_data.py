...
def generate_lane_dicts(tf) -> dict:
    ...
    for lane_index, lane_id in enumerate(
        tf.sumo.trafficlight.getControlledLanes(tf.id)
    ):
    ...
    for lane_index, lane_id in enumerate(
        list(set(tf.sumo.trafficlight.getControlledLanes(tf.id)))
    ):
...
def generate_phase_data(tf, lanes: list) -> dict:
    ...
    for phase_id, phase in enumerate(tf.green_phases):
        lanes_green = set()
        for lane_id, char in enumerate(phase.state):
            if char.lower() == "g":
                lanes_green.add(lanes[lane_id])
